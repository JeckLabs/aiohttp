...
class CookieJar(http.cookiejar.CookieJar):
    ...
    def __init__(self, policy=None, loop=None, unsafe=False):
        super(CookieJar, self).__init__(policy)
    ...
    def _get_response(self, headers):
        dummy_headers = {
            'get_all': lambda key, default: headers.getall(key, default)
        }
        dummy_headers = type('Headers', (object,), dummy_headers)
        dummy_response = {
            'info': lambda: dummy_headers
        }
        dummy_response = type('Response', (object,), dummy_response)
    ...
    def _get_request(self, url):
        ...
        d = {
            ...
            'type': o.scheme,
            'unverifiable': True,
            'origin_req_host': o.netloc,
            'get_type': lambda: o.scheme,
            'get_host': lambda: o.netloc,
            'get_origin_req_host': lambda: o.netloc,
            'get_full_url': lambda: url,
            'is_unverifiable': lambda: True,
            'has_header': lambda name: False,
            'get_header': lambda name, default=None: default,
            'get_cookie_headers': lambda: d.new,
            'new': {},
        }
